...
if (isset($_POST['add-post'])) {
    ...
    if (mysqli_num_rows($res) > 0) {
        ...
        //htmlspecialchars преобразует двойные кавычки и опострофы (которы могут повлиять на sql запрос)
        //на те, ктороые не могут повлиять
...
if (isset($_POST['add-field'])) {
    ...
    foreach ($hash_ids as $hash_id) {
        $sql = "INSERT INTO `hash_conn` (`hash_id`, `field_id`) VALUES (
...
if (isset($_POST['update-field'])) {
    ...
    $sql = "DELETE FROM `hash_conn` WHERE `field_id` = '$field_id'";
    mysqli_query($connect, $sql);
